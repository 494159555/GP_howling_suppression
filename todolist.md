...
- **位置**: 第2章 2.1.2节末尾，"图2-1 声反馈正反馈过程示意图"标题处
- **来源文件**: `02_chapter.md` 第 49-53 行
- **现状**: pandoc 转换后为原始 LaTeX `\boxed{...}` 公式，在 Word 中显示为一串不可读文本
- **操作**: 删除 Word 中的 `\boxed{...}` 公式文本，手动插入信号流程框图（用 PPT/Visio/draw.io 绘制：麦克风→放大器(G)→扬声器→声学路径h(t)→反馈信号d(t)→回到麦克风，形成闭环）
